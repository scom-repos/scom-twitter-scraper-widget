...
class TwitterManager {
    ...
    async init() {
        ...
        this._page = page;
        const storedCookies = fs_1.default.readFileSync(path_1.default.join(process.cwd(), 'twitter_cookies.json'));
        if (storedCookies) {
        ...
        if (!isLogin) {
            ...
            if (loginSuccess) {
                ...
                fs_1.default.writeFileSync(path_1.default.join(process.cwd(), 'twitter_cookies.json'), JSON.stringify(cookies, null, 2));
            }
        }
    }
    async checkIsLogin() {
